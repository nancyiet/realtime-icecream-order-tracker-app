...
let status = document.querySelectorAll('.order-tracking')
...
function updateStatus(order)
{
    ...
    status.forEach(currStatus=>{
        let dataStatus = currStatus.dataset.status;
        if(stepCompleted)
        {
            currStatus.classList.add('completed');
            
        }
        console.log(dataStatus , order.status)
        if(dataStatus === order.status)
        { 
            time.innerText= moment(order.updatedAt).format('hh:mm A');
            currStatus.append(time);
            stepCompleted = false;   
        }
    })
}
...
socket.on('adminOrderUpdated',data=>{
    new Noty({
        type:'success',
        timeout:'1000',
        progressBar: false,
